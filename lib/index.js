...
export function apply(ctx) {
  ...
  function finishTask(sessionId, opts) {
    ...
    if (!silent) pushReminder('finish', sessionId, '✅ 任务结束：' + base + '，' + plan + diff)
  ...
  function externalAlert(title, message) {
    try {
      ...
      const args = ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', TOAST_SCRIPT, '-Title=' + safeTitle, '-Message=' + safeMessage]
    ...
    try {
      ...
      for (const [sessionId, t] of tasks) {
        ...
        if (due > t.remindersFired) {
          ...
          if (c.externalAlert) {
            const task = c.taskName || '未命名任务'
            externalAlert('⏰ ' + task + ' · 第 ' + t.remindersFired + ' 次提醒', '已运行 ' + fmtClock(elapsed) + '（' + plan + '）')
          }
        }
        if (t.plannedMs && elapsed >= t.plannedMs && !t.overdueFired) {
          ...
          if (c.externalAlert) {
            const task = c.taskName || '未命名任务'
            externalAlert('⚠️ ' + task + ' · 超时', '已超计划 ' + fmtMin(t.plannedMs) + '，当前运行 ' + fmtClock(elapsed) + '。请判断是否中止或调整。')
          }
        }
      }
    } catch (e) {}
  ...
  const handlers = {
    ...
    'get-task-board': async () => {
      ...
      // 判断每个已完成记录的会话是否仍然存活（当前活跃任务 + agents.roots()）
