...
export function apply(ctx) {
  ...
  // ---------- toast click-to-jump (second-instance + file fallback) ----------
  // 用户点击 Windows 系统 toast 时，系统通过激活当前实例（传参 dshjump:<sessionId>）
  // 以此触发 Electron second-instance 事件，或冷启动 process.argv 捕获。
  // 这些处理器将 sessionId 写入内存变量 + 临时文件。
  // 客户端每 2 秒轮询 get-pending-jump RPC → consumeToastJump() 读取并消费，
  // 调用 jumpToSession() 完成跳转。
  // 文件同时作为持久化兜底：即使客户端在事件到来时尚未启动（冷启动场景），
  // 后续首次轮询也能从文件获取跳转请求。若 combined-instance 不可靠，
  // 文件 + 内存双保险仍能保证跳转；文件 5 分钟后过期避免残留。
  let pendingToastJump = null // 保留内存变量供 cold-start process.argv 兜底
  function writeToastJump(sessionId) {
    try {
      if (!sessionId) return
      mkdirSync(dirname(JUMP_FILE), { recursive: true })
      writeFileSync(JUMP_FILE, JSON.stringify({ sessionId, ts: Date.now() }, null, 0), 'utf8')
    } catch (e) {}
  }
  function consumeToastJump() {
    // 返回待处理的跳转请求（sessionId），消费后即从文件+内存删除。
    // 优先检查内存变量（由冷启动 process.argv 设置，或 second-instance 事件设置）；
    // 内存有值则一并清理文件（防止下次轮询重复消费）；其次检查文件。
    const memSid = pendingToastJump
    if (memSid) {
      pendingToastJump = null
      try { if (existsSync(JUMP_FILE)) unlinkSync(JUMP_FILE) } catch (e) {}
      return memSid
    }
    try {
  ...
  // 冷启动时检查 process.argv（应用由 toast 点击冷启动，传参 dshjump:<sessionId>）
  for (const arg of process.argv) {
    const m = String(arg).match(/^dshjump:(.+)$/)
    if (m) { pendingToastJump = m[1]; writeToastJump(m[1]); break }
  }
  // 如果拿到 _electronApp，仍注册 second-instance 作为额外兜底
  let _toastJumpListener = null
  const _dshTaskTimeRequire = createRequire(import.meta.url)
  let _electronApp = null
  try { const _e = _dshTaskTimeRequire('electron'); if (_e && typeof _e.app === 'object') _electronApp = _e.app } catch (e) {}
  if (_electronApp) {
  ...
  function externalAlert(title, message, sessionId, sound) {
    try {
      ...
      // 写跳转请求到文件，供客户端轮询消费（替代不可靠的 second-instance 事件）
      // 注意：writeToastJump 已从 send 时移除，仅在用户实际点击系统通知时写入。
      // 由 second-instance 事件或冷启动 process.argv 处理程序触发写入。
      // if (sessionId) writeToastJump(sessionId)
