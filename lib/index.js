...
const defaults = { reminderIntervalMinutes: 10, plannedMinutes: 60, toastAppId: 'ai.deepseek.dsh.desktop', externalWhen: 'unfocused' }
...
export function apply(ctx) {
  ...
  function shouldSendExternal() {
    if (!isModuleOn('alert', 'externalNotify')) return false
    return !isDshInFront()
  }
  ...
  const handlers = {
    ...
    'set-config': (args) => {
      ...
      if (Number.isFinite(cfg.plannedMinutes) && cfg.plannedMinutes > 0) defaults.plannedMinutes = Math.round(cfg.plannedMinutes)
