...
export function apply(ctx) {
  ...
  // ---------- toast click-to-jump (file-based, bypasses second-instance) ----------
  // toast.ps1 保持存活并监听 toast 的 Activated 事件：用户点击时直接写入
  // JUMP_FILE（~/.dsh/dsh-task-time-pending-jump.json），然后激活 DSH 窗口。
  // 客户端每 2 秒轮询 get-pending-jump RPC → consumeToastJump() 读取并消费，
  // 调用 jumpToSession() 完成跳转。此机制完全绕过 Electron second-instance
  // 事件链（main process 不转发 argv 到 renderer/plugin process），
  // 也无需写跳转文件的 PowerShell 侧 second-instance 回调。
  // 文件 5 分钟后过期避免残留。
  function consumeToastJump() {
    // 返回待处理的跳转请求（sessionId），消费后即删除临时文件。
    // 文件由 toast.ps1 在用户点击通知时写入，绕过了不可靠的 second-instance 事件。
    try {
