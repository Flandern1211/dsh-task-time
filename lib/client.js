...
(() => {
  ...
  function TaskBoard() {
    ...
    var activeRows = (data.active || []).map(function (a) {
      ...
      var dropBtn = a.sessionGone
        ...
        : null;
      return React.createElement("div", { key: a.sessionId, className: "tt-board-item" + (a.sessionGone ? " tt-bi-gone-item" : ""), onClick: function () { goToSession(a.sessionId); }, title: "点击打开该任务会话" },
  ...
  function apply(ctx) {
    ...
    document.addEventListener('visibilitychange', function () {
      if (document.visibilityState === 'visible' && titleFlashActive) {
        document.title = originalTitle;
      } else if (document.visibilityState === 'visible' && !hasPendingDecision) {
        document.title = originalTitle;
      }
    });
    // 请求通知权限（页面加载后静默请求）
    ...
    // ---------- DOM 监听审批弹窗兜底 ----------
    // 当 DSH 显示 allow/deny 审批弹窗（[data-approval-key]）时触发通知。
    // 使用 MutationObserver 比 ctx.remote.$on 更可靠（事件可能被上游 handler 消费）。
    var approvalObserver = null;
    var lastApprovalKey = null;
    function setupApprovalObserver() {
      if (approvalObserver) return;
      var target = document.getElementById('root') || document.body;
      approvalObserver = new MutationObserver(function () {
        var el = document.querySelector('[data-approval-key]');
        if (el) {
          var key = el.getAttribute('data-approval-key');
          if (key && key !== lastApprovalKey) {
            lastApprovalKey = key;
            // 从当前会话获取 sessionId
            var sid = null;
            try {
            ...
            if (sid) {
              ...
              updateTitleFlash();
              // 浏览器通知
              sendBrowserNotif('❓ 需要你审批', 'Agent 请求执行需要你批准的工具', sid);
              // 触发 host 外部 toast + 循环报警音
              rpc('trigger-decision-alert', { sessionId: sid, toolName: '?' }).catch(function () {});
            }
          }
        }
      });
      approvalObserver.observe(target, { childList: true, subtree: true, attributes: true, attributeFilter: ['data-approval-key'] });
    }
    setupApprovalObserver();
    // 也监听 React 动态添加的 approval 弹窗（重新挂载后重新观察）
    var rootObserver = new MutationObserver(function () { setupApprovalObserver(); });
    rootObserver.observe(document.getElementById('root') || document.body, { childList: true, subtree: false });
    ...
    var poll = setInterval(function () {
      ...
      // 外部 toast 点击跳转：主进程捕获点击后将 sessionId 通过 RPC 暴露，
      // 客户端轮询发现后立即跳转，并从操作中心移除对应通知。
