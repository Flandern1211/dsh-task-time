...
(() => {
  ...
  function pushReminder(r) {
    ...
    // ===== 决策提醒：直接用 DOM 渲染卡片，绕过 React 管线 =====
    if (r.needDecision && r.sessionId) {
      try {
        var container = document.getElementById('__tt_cards');
        if (!container) {
          container = document.createElement('div');
          container.id = '__tt_cards';
          container.style.cssText = 'position:fixed;bottom:16px;right:16px;z-index:99990;display:flex;flex-direction:column-reverse;gap:8px;max-width:380px;';
          document.body.appendChild(container);
        }
        // 移除同一会话的旧决策卡
        var existing = container.querySelectorAll('[data-tt-session]');
        for (var ei = 0; ei < existing.length; ei++) {
          if (existing[ei].getAttribute('data-tt-session') === String(r.sessionId)) {
            existing[ei].parentNode && existing[ei].parentNode.removeChild(existing[ei]);
          }
        }
        // 创建新卡片
        var card = document.createElement('div');
        card.className = 'tt-reminder tt-need-decision tt-clickable';
        card.setAttribute('data-tt-session', String(r.sessionId));
        card.setAttribute('data-tt-id', String(r.id));
        card.style.cssText = 'background:var(--dsw-bg-primary,#1e1e2e);border-left:4px solid #f87171;border-radius:8px;padding:10px 12px;box-shadow:0 6px 24px rgba(0,0,0,.35);font-size:13px;line-height:1.5;cursor:pointer;';
        card.title = '点击跳转到对应会话';
        card.onclick = function (ev) {
          if (ev && ev.target && ev.target.className && String(ev.target.className).indexOf('tt-close') >= 0) return;
          jumpToSession(r.sessionId);
          if (card.parentNode) card.parentNode.removeChild(card);
        };
        var head = document.createElement('div');
        head.className = 'tt-reminder-head';
        head.style.cssText = 'color:#f87171;font-weight:600;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center;';
        var title = document.createElement('span');
        title.textContent = '❓ ' + (r.taskName || '任务') + '：需要你决策';
        ...
        closeBtn.onclick = function (e) { e.stopPropagation(); if (card.parentNode) card.parentNode.removeChild(card); };
        head.appendChild(title);
        head.appendChild(closeBtn);
        var body = document.createElement('div');
        body.textContent = r.text || '';
        body.style.cssText = 'color:#ccc;margin-bottom:4px;';
        var foot = document.createElement('div');
        foot.textContent = '点击跳转到会话 →';
        foot.style.cssText = 'color:#888;font-size:11px;';
  ...
  function SetupManager() {
    ...
    var onBackdropClick = function () {
      setHint("设置窗口不会因为点到外面而关闭：点「开始」保存设置，或点「跳过」放弃本次设置。");
      if (hintTimerRef.current) clearTimeout(hintTimerRef.current);
      hintTimerRef.current = setTimeout(function () { setHint(""); }, 5000);
    };
  ...
  function SettingsSection() {
    var cfgState = React.useState({ reminderIntervalMinutes: 10, plannedMinutes: 60, toastAppId: "", externalWhen: "unfocused" });
    var cfg = cfgState[0];
    ...
    React.useEffect(function () {
      var alive = true;
      rpc("get-config", {}).then(function (d) { if (alive) setCfg(Object.assign({ reminderIntervalMinutes: 10, plannedMinutes: 60, toastAppId: "", externalWhen: "unfocused" }, d)); }).catch(function () {});
    ...
    function testToast() {
      rpc("test-toast", {}).then(function () {
        pushReminder({ id: -Date.now(), kind: "test", taskName: "测试通知已发送", text: "系统通知已发出（身份：" + (cfg.toastAppId || "ai.deepseek.dsh.desktop") + "）。测试不受「外部通知时机」限制；若未看到横幅，请检查 Windows 通知设置/专注助手。", ts: Date.now(), needDecision: false, autoMs: 8000 });
    ...
    // 提醒模块的配置：外部通知（开关在模块开关里，这里配置「什么时候发」与身份/测试）
    var alertCfgRows = [
      subHead("alert-cfg-head", "外部通知配置（人不在 DSH 里时才发）"),
      ...
      return [
        ...
        slots.register({ name: "shell.overlay", id: "task-time-reminder", order: 60, label: "任务提醒" },
          function ReminderGate(props) {
            React.useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
            if (!isModuleOn('alert', 'reminderUI') && !isModuleOn('timing', 'intervalReminder')) return null;
