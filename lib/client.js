...
(() => {
  ...
  .tt-setup-link:hover { background: rgba(234,179,8,.15); }
  ...
  /* 模块内的配置行：不是开关，去掉行尾分隔线与整行 padding */
  .tt-settings .tt-row.tt-cfg-row > label {
    cursor: default;
    color: var(--dsw-alias-label-secondary, #6b7280);
  }
  .tt-settings select.tt-select {
    padding: 4px 6px;
    border: 1px solid var(--dsw-alias-border-l1, #e2e5ea);
    border-radius: 6px;
    font-size: 13px;
  ...
  .tt-settings .tt-row input[type="checkbox"] {
    margin-right: 6px;
    cursor: pointer;
  }
  .tt-settings .tt-row label {
    cursor: pointer;
    user-select: none;
  }
  `;
  ...
  var _snapshot = { reminders: [], boardOpen: false, boardData: { active: [], finished: [], file: null }, moduleConfig: null };
  ...
  // React 18 的 useSyncExternalStore 在 hydration 期间要求第三个参数（getServerSnapshot），
  // 缺了它会在首次渲染抛错，DSH 的 slot 错误边界会把该 entry 永久摘掉（abdicate）。
  // 固定返回「配置未加载」的空快照：外壳的 boot DOM 里没有任何插件 UI，必须与之一致。
  var SERVER_SNAPSHOT = Object.freeze({ reminders: [], boardOpen: false, boardData: { active: [], finished: [], file: null }, moduleConfig: null });
  function getServerSnapshot() {
    return SERVER_SNAPSHOT;
  }
  function emitChange() {
    _snapshot = { reminders: store.reminders, boardOpen: store.boardOpen, boardData: store.boardData, moduleConfig: store.moduleConfig };
  ...
  // 注意：此函数在模块级定义，不能访问 apply() 内部的变量（如 updateTitleFlash）。
  ...
  // ---------- reminder store ops ----------
  // 提醒卡滞留时长由 host 配置决定（设置页「提醒 → 内部通知配置 → 提醒卡滞留时长」）。
  // 默认 4 秒，对齐 DSH 内置 Toast（HOLD_MS=3s + FADE_MS=1s）；
  // host 下发 autoMs（毫秒）后，client 在此之上加 1s 淡出再移除。
  // autoMs === 0 表示常驻（「需要你决策」提醒，以及用户把滞留时长设为「常驻」）：
  // 不自动消散，只能点 × 关闭。
  var FADE_MS = 1000;
  // 本地兜底值：host 未下发时用它（与 host 的默认值一致）
  var DEFAULT_CARD_HOLD_MS = 4000;
  var cardHoldMs = DEFAULT_CARD_HOLD_MS;
  var reminderTimers = new Map();
  var fadingIds = new Set();

  // 把 host 的配置同步成本地的兜底滞留时长（client 本地生成的提醒卡——删除失败/测试通知
  // 等——不带 autoMs，就按这个值消散；host 下发的 autoMs 始终优先）。
  function syncCardHoldMs(cfg) {
    if (!cfg) return;
    var s = Number(cfg.reminderAutoDismissSeconds);
    if (isFinite(s) && s >= 0) cardHoldMs = Math.round(s) * 1000;
  }
  ...
  function pushReminder(r) {
    ...
    // 同一会话只保留一张「需要你决策」卡：本地 approval 检测（负 id）与 host 轮询
    // （正 id）会各自推一条，不去重就会在一个会话上叠出两张一样的红卡。
    if (r && r.needDecision && r.sessionId) {
      dismissDecisionForSession(r.sessionId);
    }
    // 决策已处理（host 推 decision-cleared）：把该会话的常驻红卡收掉，
    // 否则用户处理完审批，红卡还挂在界面上、标题也继续闪「需要你决策」。
    if (r && r.kind === "decision-cleared" && r.sessionId) {
      dismissDecisionForSession(r.sessionId);
    }
    store.reminders = [r].concat(store.reminders.filter(function (x) { return x.id !== r.id; }));
    if (store.reminders.length > 8) {
      // 被挤掉的卡片要连它的淡出定时器一起清掉，否则定时器会一直留在 Map 里
      var dropped = store.reminders.slice(8);
      store.reminders = store.reminders.slice(0, 8);
      dropped.forEach(function (x) {
        var dt = reminderTimers.get(x.id);
        if (dt) { clearTimeout(dt); reminderTimers.delete(x.id); }
        fadingIds.delete(x.id);
      });
    }
    var holdMs = typeof r.autoMs === "number" && isFinite(r.autoMs) ? r.autoMs : cardHoldMs;
    if (holdMs > 0) {
  ...
  // ---------- 内部通知 / 外部通知 两条通道 ----------
  //   人就在 DSH 里 → 内部通知：右下角提醒卡 + 提示音（这里用 WebAudio 合成，不依赖外部文件）
  //   人不在 DSH 里 → 外部通知：host 发 Windows Toast（+ 浏览器通知）
  // 「在不在 DSH」由 host 统一判定（它按 externalWhen 配置来算），client 从 ui-presence 的
  // 返回值同步到本地 inFront。这样两边结论一致：响了提示音就不会再发系统通知，反之亦然。
  var inFront = false;
  var audioCtx = null;
  var lastChimeAt = 0;
  ...
  function playInternalChime(urgent) {
    ...
    // 需要决策：三声上行短音；普通提醒：两声
    var notes = urgent ? [880, 1174, 1568] : [784, 1046];
    var gap = urgent ? 0.15 : 0.17;
    for (var i = 0; i < notes.length; i++) {
      try {
        var t0 = ac.currentTime + 0.02 + i * gap;
        var osc = ac.createOscillator();
        var gain = ac.createGain();
        osc.type = 'sine';
        osc.frequency.value = notes[i];
        gain.gain.setValueAtTime(0.0001, t0);
        gain.gain.exponentialRampToValueAtTime(urgent ? 0.2 : 0.14, t0 + 0.015);
        gain.gain.exponentialRampToValueAtTime(0.0001, t0 + gap * 0.85);
        osc.connect(gain);
  ...
  // 内部通知入口：提醒卡由 pushReminder 负责显示，这里只负责「在 DSH 里的时候响一声」
  function notifyInternal(r) {
    if (!r) return;
    if (!isModuleOn('alert', 'reminderUI')) return;
    if (!inFront) return; // 人不在 DSH → 交给外部通知（系统 Toast），不重复响
    var now = Date.now();
    if (now - lastChimeAt < 1200) return; // 一次来好几条提醒时只响一次
    lastChimeAt = now;
    playInternalChime(!!r.needDecision);
  }

  // Chromium 的自动播放策略：先有用户交互才允许出声。点一下/敲一下键盘就解锁。
  ...
  function reportPresence(force) {
    ...
    var key = (focused ? '1' : '0') + (visible ? '1' : '0');
    var now = Date.now();
    // 状态没变也至少每 5 秒续报一次（host 用 10 秒超时判定「页面已经关掉/停了」）
    if (!force && key === lastPresenceKey && now - lastPresenceAt < 5000) return;
    lastPresenceKey = key;
    lastPresenceAt = now;
    rpc('ui-presence', { focused: focused, visible: visible }).then(function (d) {
      if (d && typeof d.inFront === 'boolean') inFront = d.inFront;
    }).catch(function () {});
  }

  // ---------- ReminderStack ----------
  function ReminderStack() {
    var snap = React.useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
    ...
    function deleteRecord(sessionId, finishedAt, ev) {
      ...
      rpc("delete-task-record", { sessionId: sessionId, finishedAt: finishedAt }).then(function (d) {
        if (d && d.ok) {
          ...
          pushReminder({ id: -Date.now(), kind: 'error', taskName: '删除失败', text: (d && d.error) || '删除失败', ts: Date.now(), needDecision: false });
        }
      }).catch(function (e) {
        pushReminder({ id: -Date.now(), kind: 'error', taskName: '删除失败', text: '删除失败：' + String((e && e.message) || e) + '。可能需要重启 DSH Desktop 加载新版本插件。', ts: Date.now(), needDecision: false });
      });
    }
    function endTask(sessionId, ev) {
      ...
      rpc("end-task", { sessionId: sessionId }).then(function (d) {
        if (d && d.ok) {
          ...
          pushReminder({ id: -Date.now(), kind: 'ended', taskName: '任务已结束', text: '已结束任务「' + (d.record && d.record.taskName) + '」并归档到已完成。', ts: Date.now(), needDecision: false });
        } else {
          pushReminder({ id: -Date.now(), kind: 'error', taskName: '结束失败', text: (d && d.error) || '结束任务失败', ts: Date.now(), needDecision: false });
        }
      }).catch(function (e) {
        pushReminder({ id: -Date.now(), kind: 'error', taskName: '结束失败', text: '结束任务失败：' + String((e && e.message) || e), ts: Date.now(), needDecision: false });
      });
    }
    function dropTask(sessionId, ev) {
      ...
      rpc("drop-task", { sessionId: sessionId }).then(function (d) {
        if (d && d.ok) {
          ...
          pushReminder({ id: -Date.now(), kind: 'dropped', taskName: '任务已移除', text: '已移除残留任务「' + (d.taskName || '') + '」。', ts: Date.now(), needDecision: false });
        } else {
          pushReminder({ id: -Date.now(), kind: 'error', taskName: '移除失败', text: (d && d.error) || '移除失败', ts: Date.now(), needDecision: false });
        }
      }).catch(function (e) {
        pushReminder({ id: -Date.now(), kind: 'error', taskName: '移除失败', text: '移除失败：' + String((e && e.message) || e), ts: Date.now(), needDecision: false });
      });
    }
    function clearDecision(sessionId, ev) {
      ...
      rpc("clear-decision", { sessionId: sessionId }).then(function (d) {
        if (d && d.ok) {
          ...
          pushReminder({ id: -Date.now(), kind: 'decision-cleared', taskName: '决策已清除', text: '已清除该任务的“需决策”状态。', ts: Date.now(), needDecision: false });
        }
      }).catch(function () {});
    }
  ...
  function DockEntry(props) {
    ...
    if (status && status.active) {
      ...
      // 会话被跳过、或还没设置时，给一个回到设置弹窗的入口：跳过不再是不可逆的死路。
      statusEl = React.createElement("div", { className: "tt-strip" },
        React.createElement("span", { className: "tt-paused" }, "⏱ 未设置计划用时"),
        React.createElement("button", {
          className: "tt-setup-link",
          title: "重新打开「设置任务计划」弹窗",
          onClick: function (ev) {
            if (ev && ev.stopPropagation) ev.stopPropagation();
            rpc("reopen-session-setup", { sessionId: sessionId })
              .then(function () { setStatus(Object.assign({}, status, { configured: true })); })
              .catch(function () {});
          }
        }, "⚙ 设置计划")
      );
  ...
  function SetupManager() {
    ...
    var hintTimerRef = React.useRef(null);
          ...
          if (d && d.pending) {
            ...
            setSetup(function (prev) {
              ...
              return {
                ...
                // 设置页「任务用时 → 默认配置」里可改的默认计划用时（host 下发）
                defaultPlannedMinutes: d.defaultPlannedMinutes || 60,
    ...
    React.useEffect(function () {
      ...
      // 计划用时预填优先级：会话已填过的值 → 设置页里的默认计划用时 → 60
      setPlan(String(setup.plannedMinutes || setup.defaultPlannedMinutes || "60"));
      setInterval(String(setup.reminderIntervalMinutes || "10"));
      setHint("");
    }, [setup ? setup.sessionId : null]);

    React.useEffect(function () {
      return function () { if (hintTimerRef.current) clearTimeout(hintTimerRef.current); };
    }, []);

    if (!setup) return null;
    var skip = function () { rpc("dismiss-session-setup", { sessionId: setup.sessionId }).then(function () { setSetup(null); }).catch(function () {}); };
    var showHint = function (text, ms) {
      setHint(text);
      if (hintTimerRef.current) clearTimeout(hintTimerRef.current);
      hintTimerRef.current = setTimeout(function () { setHint(""); }, ms || 5000);
    };
    // 遮罩（弹窗外区域）点击不再等于「跳过」。
    // 原因：跳过是不可逆的（会话被永久标记为已跳过、配置被清除），而遮罩覆盖了除卡片外的整个窗口，
    // 两类误触都会把设置吞掉：
    //   1) 按下在卡片内、松开在卡片外时，浏览器把 click 派发到两者最近的公共祖先＝遮罩本身
    //      （卡片上的 stopPropagation 挡不住，因为事件目标就是遮罩）；
    //   2) 鼠标滑出窗口后，为了把窗口点回前台而点的那一下，正好落在遮罩上。
    // 所以遮罩点击只给提示，只有「跳过」按钮才会真正跳过。
    var onBackdropClick = function () {
      showHint("设置窗口不会因为点到外面而关闭：点「开始」保存设置，或点「跳过」放弃本次设置。");
    };
    // 「开始」：先在本地校验，避免把空/非法值发给 host 后按钮像坏了一样没反应
    // （host 也会再校验一次；计划用时是必须项，缺了会返回 ok:false）
    var onStart = function () {
      var p = parseInt(plan, 10);
      if (!isFinite(p) || p <= 0) {
        showHint("请填写计划用时（分钟，至少 1 分钟）——留空的话任务不会开始计时。");
        return;
      }
      var iv = parseInt(interval, 10);
      if (!isFinite(iv) || iv <= 0) {
        showHint("请填写提醒间隔（分钟，至少 1 分钟）。");
        return;
      }
      rpc("set-session-config", {
        sessionId: setup.sessionId,
        taskName: taskName.trim(),
        plannedMinutes: p,
        reminderIntervalMinutes: iv
      }).then(function () { setSetup(null); }).catch(function (e) {
        showHint("保存失败：" + ((e && e.message) || "请重试"));
      });
    };
    return React.createElement("div", { className: "tt-setup", onClick: onBackdropClick },
      React.createElement("div", { className: "tt-setup-card", onClick: function (ev) { if (ev && ev.stopPropagation) ev.stopPropagation(); } },
        ...
        hint ? React.createElement("div", { className: "tt-setup-hint" }, hint) : null,
        React.createElement("div", { className: "tt-setup-actions" },
          React.createElement("button", { className: "tt-setup-start", onClick: onStart }, "开始"),
          React.createElement("button", { className: "tt-setup-skip", onClick: skip }, "跳过")
        ),
        React.createElement("div", { className: "tt-setup-note" }, "计时仅在模型运行时累加，暂停不计时。只有点「跳过」才会跳过本次设置，误触不会；跳过后可在输入框下方的计时条点「设置计划」重新打开。")
      )
    );
  }
  ...
  function ModuleToggle(props) {
    // props: label, checked, onChange, indent (number of levels), disabled
    var indentPx = (props.indent || 0) * 20;
    var style = { paddingLeft: indentPx + 'px' };
    if (props.disabled) style.opacity = 0.45;
    return React.createElement("div", { className: "tt-row", style: style },
      React.createElement("label", { title: props.tooltip || '' },
  ...
  // 任务用时的「默认配置」：可修改的默认值，作为新会话「设置任务计划」弹窗的预填。
  // 默认计划用时用「常用预设下拉 + 自定义」的选择式配置，避免每次都手打分钟数。
  var PLAN_PRESET_MINUTES = [15, 30, 45, 60, 90, 120, 180];
  // 提醒卡滞留时长的常用预设（秒）。0 = 常驻不自动消散（只能点 × 关闭）。
  var CARD_PRESET_SECONDS = [2, 3, 4, 5, 8, 10, 15, 30];
  var CARD_KEEP_SECONDS = 0;

  function SettingsSection() {
    var cfgState = React.useState({ reminderIntervalMinutes: 10, plannedMinutes: 60, toastAppId: "", externalWhen: "unfocused", reminderAutoDismissSeconds: 4 });
    var cfg = cfgState[0];
    var setCfg = cfgState[1];
    var planCustomState = React.useState(false);
    var planCustom = planCustomState[0];
    var setPlanCustom = planCustomState[1];
    var cardCustomState = React.useState(false);
    var cardCustom = cardCustomState[0];
    var setCardCustom = cardCustomState[1];
    var diagState = React.useState("");
    var diag = diagState[0];
    var setDiag = diagState[1];
    var extState = React.useState(null);
    var extStatus = extState[0];
    var setExtStatus = extState[1];
    var mcState = React.useState(null);
    var mc = mcState[0];
    var setMc = mcState[1];
    ...
    // 拉一次外部通知现状（人在不在 DSH、最近诊断），并同步 host 的判定结果
    function refreshExt() {
      rpc("get-external-status", {}).then(function (d) {
        if (!d) return;
        setExtStatus(d);
        setDiag(d.toastDiag || "");
        if (typeof d.inFront === "boolean") inFront = d.inFront;
        if (d.externalWhen === "hidden" || d.externalWhen === "unfocused") {
          setCfg(function (prev) { return Object.assign({}, prev, { externalWhen: d.externalWhen }); });
        }
      }).catch(function () {});
    }
    ...
    React.useEffect(function () {
      var alive = true;
      rpc("get-config", {}).then(function (d) {
        if (!alive) return;
        setCfg(Object.assign({ reminderIntervalMinutes: 10, plannedMinutes: 60, toastAppId: "", externalWhen: "unfocused", reminderAutoDismissSeconds: 4 }, d));
        syncCardHoldMs(d); // 本地提醒卡（无 autoMs）按同一滞留时长消散
      }).catch(function () {});
      rpc("get-module-config", {}).then(function (d) { if (alive && d) setMc(d); }).catch(function () {});
      refreshExt();
      // 设置页打开期间每 5 秒刷新一次「当前状态」，切窗口/切页面能立刻看到变化
      var t = setInterval(function () { if (alive) refreshExt(); }, 5000);
      return function () { alive = false; clearInterval(t); };
    }, []);

    // 全局默认配置的保存：本地先更新，再回写 host（host 持久化到 ~/.dsh/dsh-task-time-records.json）
    function saveCfg(next) {
      setCfg(next);
      rpc("set-config", { config: next }).catch(function () {});
    }

    function setPlannedMinutes(v) {
      var n = parseInt(v, 10);
      if (!isFinite(n) || n <= 0) return;
      saveCfg(Object.assign({}, cfg, { plannedMinutes: n }));
    }

    // 提醒卡滞留时长（秒）：0 = 常驻不自动消散
    function setCardSeconds(v) {
      var n = parseInt(v, 10);
      if (!isFinite(n) || n < 0) return;
      var sec = Math.min(600, n);
      saveCfg(Object.assign({}, cfg, { reminderAutoDismissSeconds: sec }));
      syncCardHoldMs({ reminderAutoDismissSeconds: sec });
    }

    // 保存串行化：本地乐观更新 + 串行落盘，任何一次点击都不会被丢弃
    var saveChain = Promise.resolve();
    var pendingSaves = 0;
    function saveModuleConfig(next) {
      // 立即本地生效，UI 与开关状态同步（不等 RPC 往返）
      setMc(next);
      store.moduleConfig = next;
      emitChange();
      pendingSaves += 1;
      setSaving(true);
      saveChain = saveChain
        .then(function () { return rpc("set-module-config", { config: next }); })
        .then(function (d) {
          // 以 host 返回的权威配置为准（合并默认值后的结果）
          if (d && d.config) {
            setMc(d.config);
            store.moduleConfig = d.config;
            emitChange();
          }
        })
        .catch(function () {})
        .then(function () {
          pendingSaves -= 1;
          if (pendingSaves <= 0) { pendingSaves = 0; setSaving(false); }
        });
    }

    function toggleModule(moduleKey, childKey, checked) {
      if (!mc) return;
      var next = JSON.parse(JSON.stringify(mc));
      if (childKey) {
        next[moduleKey].children[childKey] = checked;
      } else {
        // 只切换主开关本身；子模块各自的状态保留（关闭时由主开关统一屏蔽，
        // 重新开启后子模块恢复原状，不会被清空）。
        next[moduleKey].enabled = checked;
      }
    ...
    function testToast() {
      rpc("test-toast", {}).then(function () {
        pushReminder({ id: -Date.now(), kind: "test", taskName: "测试通知已发送", text: "系统通知已发出（身份：" + (cfg.toastAppId || "ai.deepseek.dsh.desktop") + "）。测试不受「外部通知时机」限制；若未看到横幅，请检查 Windows 通知设置/专注助手。", ts: Date.now(), needDecision: false });
        setTimeout(refreshExt, 1500);
      }).catch(function (e) {
        pushReminder({ id: -Date.now(), kind: "error", taskName: "测试通知失败", text: String((e && e.message) || e), ts: Date.now(), needDecision: false });
      });
    }

    // 模块开关 + 该模块最需要的配置：按模块分组，配置直接放在对应模块里面
    // （任务用时 → 默认计划用时/默认提醒间隔；提醒 → 外部通知配置/测试通知）。
    function groupHead(key, text, first) {
      return React.createElement("div", { key: key, className: "tt-module-head", style: { fontWeight: 700, fontSize: 14, color: '#eab308', padding: first ? '2px 0 4px' : '14px 0 4px', borderTop: first ? 'none' : '1px solid var(--dsw-alias-border-l1, #e2e5ea)' } }, text);
    }
    function subHead(key, text) {
      return React.createElement("div", { key: key, className: "tt-module-subhead", style: { fontSize: 11, color: '#8a90a0', padding: '8px 10px 2px 28px' } }, text);
    }
    function cfgRow(key, label, node, tooltip) {
      return React.createElement("div", { key: key, className: "tt-row tt-cfg-row", style: { borderBottom: 'none', padding: '4px 10px 4px 28px' } },
        React.createElement("label", { title: tooltip || '', style: { fontSize: 12 } }, label),
        node);
    }
    function minutesInput(key, value, onChange) {
      return React.createElement("input", {
        key: key, type: "number", min: 1, style: { width: 90 }, value: value,
        onChange: function (e) { var v = parseInt(e.target.value, 10); if (isFinite(v) && v > 0) onChange(v); }
      });
    }
    // 秒输入：0 合法（= 常驻），上限与 host 的夹取范围一致（600 秒）
    function secondsInput(key, value, onChange) {
      return React.createElement("input", {
        key: key, type: "number", min: 0, max: 600, style: { width: 90 }, value: value,
        onChange: function (e) { var v = parseInt(e.target.value, 10); if (isFinite(v) && v >= 0) onChange(v); }
      });
    }

    // 默认配置始终可改（即使任务用时模块暂时关闭，也允许先把默认值配好）
    var planIsPreset = PLAN_PRESET_MINUTES.indexOf(cfg.plannedMinutes) >= 0;
    var showPlanNumber = planCustom || !planIsPreset;
    var planSelectValue = showPlanNumber ? "custom" : String(cfg.plannedMinutes);

    // 任务用时模块里最需要的配置：默认计划用时（可改的默认配置，预设选择 + 自定义）
    var timingCfgRows = [
      subHead("timing-cfg-head", "默认配置（新会话设置弹窗的预填值）"),
      cfgRow("timing-cfg-plan", "默认计划用时",
        React.createElement("div", { key: "plan-input", className: "tt-cfg-input", style: { display: 'flex', alignItems: 'center', gap: 6 } },
          React.createElement("select", {
            className: "tt-select",
            value: planSelectValue,
            title: "新会话「设置任务计划」弹窗里计划用时的预填值",
            onChange: function (e) {
              var v = e.target.value;
              if (v === "custom") { setPlanCustom(true); return; }
              setPlanCustom(false);
              setPlannedMinutes(v);
            }
          },
            PLAN_PRESET_MINUTES.map(function (m) { return React.createElement("option", { key: m, value: String(m) }, m + " 分钟"); }),
            React.createElement("option", { key: "custom", value: "custom" }, "自定义…")
          ),
          showPlanNumber ? minutesInput("plan-custom-num", cfg.plannedMinutes, setPlannedMinutes) : null
        ),
        "新会话「设置任务计划」弹窗里计划用时的预填值"),
      cfgRow("timing-cfg-interval", "默认提醒间隔（分钟）",
        minutesInput("interval-num", cfg.reminderIntervalMinutes, function (v) { saveCfg(Object.assign({}, cfg, { reminderIntervalMinutes: v })); }),
        "每隔多少分钟（按累计运行时间）提醒一次；也是新会话设置弹窗的预填值"),
    ];

    // 提醒模块的配置：内部通知（提醒卡滞留时长）+ 外部通知（开关在模块开关里，这里配置「什么时候发」与身份/测试）
    var cardSec = cfg.reminderAutoDismissSeconds;
    var cardIsPreset = CARD_PRESET_SECONDS.indexOf(cardSec) >= 0 || cardSec === CARD_KEEP_SECONDS;
    var showCardNumber = cardCustom || !cardIsPreset;
    var cardSelectValue = showCardNumber ? "custom" : String(cardSec);

    var alertCfgRows = [
      subHead("alert-cfg-card-head", "内部通知配置（在 DSH 里：提醒卡 + 提示音）"),
      cfgRow("alert-cfg-card-dismiss", "提醒卡滞留时长",
        React.createElement("div", { key: "card-input", className: "tt-cfg-input", style: { display: 'flex', alignItems: 'center', gap: 6 } },
          React.createElement("select", {
            className: "tt-select",
            value: cardSelectValue,
            title: "提醒卡显示多久后自动淡出消散（单位：秒）",
            onChange: function (e) {
              var v = e.target.value;
              if (v === "custom") { setCardCustom(true); return; }
              setCardCustom(false);
              setCardSeconds(v);
            }
          },
            CARD_PRESET_SECONDS.map(function (s) { return React.createElement("option", { key: s, value: String(s) }, s + " 秒"); }),
            React.createElement("option", { key: "keep", value: "0" }, "常驻（不自动消散）"),
            React.createElement("option", { key: "custom", value: "custom" }, "自定义…")
          ),
          showCardNumber ? secondsInput("card-custom-num", cfg.reminderAutoDismissSeconds, setCardSeconds) : null
        ),
        "普通提醒卡（定时/超时/结束/设置完成）显示多久后自动淡出消散，默认 4 秒。「常驻（不自动消散）」= 一直留在右下角，只能点 × 或点击卡片关闭；「需要你决策」红卡始终常驻，不受这里影响。"),
      subHead("alert-cfg-ext-head", "外部通知配置（人不在 DSH 里时才发）"),
      cfgRow("alert-cfg-when", "外部通知时机",
        React.createElement("select", {
          className: "tt-select",
          value: cfg.externalWhen === "hidden" ? "hidden" : "unfocused",
          title: "决定什么时候发 Windows 系统通知",
          onChange: function (e) {
            var next = Object.assign({}, cfg, { externalWhen: e.target.value });
            setCfg(next);
            rpc("set-config", { config: next }).then(function () { setTimeout(refreshExt, 200); }).catch(function () {});
          }
        },
          React.createElement("option", { key: "unfocused", value: "unfocused" }, "DSH 不在前台时"),
          React.createElement("option", { key: "hidden", value: "hidden" }, "只有 DSH 页面不可见时（推荐）")
        ),
        "「DSH 不在前台时」＝切到别的窗口/最小化/关掉都算不在 DSH；「只有页面不可见时」＝只要 DSH 还看得见就不发系统通知。人在 DSH 里一律只走内部通知（提醒卡 + 提示音）。"),
      cfgRow("alert-cfg-state", "当前状态",
        React.createElement("span", { key: "state", style: { fontSize: 12, color: extStatus && extStatus.inFront ? '#4ade80' : '#eab308' } },
          !extStatus ? "读取中…" : (extStatus.inFront ? "在 DSH 里 → 只发内部通知" : "不在 DSH → 会发系统通知")),
        "由 client 每 2 秒上报焦点/可见性给 host，host 按上面的时机判定"),
      cfgRow("alert-cfg-aumid", "通知应用身份（AUMID）",
        ...
        "Windows 通知使用的应用身份（AUMID）。默认借用 DSH Desktop 的已注册身份；留空即默认。"),
      cfgRow("alert-cfg-test", "外部通知链路",
        React.createElement("button", { className: "tt-btn", onClick: testToast }, "发送测试通知"),
        "手动发一条系统通知，验证外部通知链路（Toast + 提示音）是否正常。测试按钮不受时机限制，随时可点。"),
    ];

    var moduleRows = [];
    moduleRows.push(groupHead("timing-head", "任务用时", true));
    if (mc) {
      moduleRows.push(React.createElement(ModuleToggle, { key: "timing-main", label: "启用任务用时", checked: mc.timing.enabled, onChange: function (v) { toggleModule('timing', null, v); } }));
      var timingChildren = [
        { key: 'timer', label: '计时引擎（运行/暂停连续计时）' },
        { key: 'planning', label: '计划配置（设置弹窗 / 跳过恢复）' },
        { key: 'intervalReminder', label: '定时提醒（间隔提醒 + 超时提醒）' },
        { key: 'taskBoard', label: '任务面板（进行中/已完成 + 操作）' },
        { key: 'dockStatus', label: 'Dock 状态条（输入框下方实时显示）' },
        { key: 'orphanCleanup', label: '孤儿清理（会话已删除的残留清理）' },
      ];
      for (var tc of timingChildren) {
        moduleRows.push(React.createElement(ModuleToggle, {
          key: 'timing-' + tc.key,
          label: tc.label,
          checked: mc.timing.children[tc.key],
          indent: 1,
          disabled: !mc.timing.enabled,
          onChange: function (key) { return function (v) { toggleModule('timing', key, v); }; }(tc.key)
        }));
      }
    }
    moduleRows = moduleRows.concat(timingCfgRows);

    moduleRows.push(groupHead("alert-head", "提醒", false));
    if (mc) {
      moduleRows.push(React.createElement(ModuleToggle, { key: "alert-main", label: "启用提醒", checked: mc.alert.enabled, onChange: function (v) { toggleModule('alert', null, v); } }));
      var alertChildren = [
        { key: 'decisionDetection', label: '决策检测（三通道 + 客户端独立检测）' },
        { key: 'reminderUI', label: '内部通知（在 DSH 里：提醒卡 + 提示音）' },
        { key: 'externalNotify', label: '外部通知（不在 DSH 里：系统 Toast + AUMID）' },
        { key: 'titleFlash', label: '标题闪烁（需要决策时）' },
        { key: 'toastJump', label: 'Toast 点击跳转（文件兜底 + 轮询消费）' },
      ];
      for (var ac of alertChildren) {
        moduleRows.push(React.createElement(ModuleToggle, {
          key: 'alert-' + ac.key,
          label: ac.label,
          checked: mc.alert.children[ac.key],
          indent: 1,
          disabled: !mc.alert.enabled,
          onChange: function (key) { return function (v) { toggleModule('alert', key, v); }; }(ac.key)
        }));
      }
    }
    moduleRows = moduleRows.concat(alertCfgRows);

    return React.createElement("div", { className: "tt-settings" },
      React.createElement("div", { className: "tt-row", style: { borderBottom: 'none' } },
        React.createElement("label", { style: { fontWeight: 600, fontSize: 13 } }, "模块开关与默认配置"),
        saving ? React.createElement("span", { style: { fontSize: 11, color: '#8a90a0' } }, "保存中…") : null
      ),
      React.createElement("div", { className: "tt-module-config" }, moduleRows),
      diag ? React.createElement("div", { className: "tt-hint" }, "最近通知诊断：" + diag) : null,
      React.createElement("div", { className: "tt-hint" },
        "通知分两条通道：人在 DSH 里 → 内部通知（右下角提醒卡 + 提示音），不发系统通知；人不在 DSH 里（切走/最小化/关掉）→ 外部通知（Windows 系统通知 + 浏览器通知）。外部通知的时机在「提醒 → 外部通知配置」里选；测试按钮不受时机限制。任务用时 → 默认配置可修改新会话的默认计划用时与默认提醒间隔。提醒 → 内部通知配置可修改提醒卡滞留时长（秒，默认 4 秒；选「常驻」则一直显示到手动关闭）。计时为连续累计：只有模型运行时才计时，暂停（等待你的输入）不计时，恢复后接着累计。创建新会话时可填写任务名并设置计划与提醒间隔（也可直接跳过）；提醒卡片右下角堆叠显示，按上面配置的滞留时长自动消散；点击提醒卡片可直接跳转到对应会话（卡片随即消失），也可点 × 手动关闭；DSH 需要你决策时弹出红色「需要你决策」提醒（常驻不自动消散 + 系统桌面通知）；任务结束自动统计累计实际用时与计划用时的差距，并写入本地任务记录文件。左下角「🗂 任务面板」可查看全部任务记录（进行中 / 已完成分组），点击任意任务即可跳转到该任务对应的会话对话。"
      )
    );
  }
  ...
  function isModuleOn(moduleKey, childKey) {
    var mc = store.moduleConfig;
    if (!mc) return false; // 配置未加载时不渲染，等待 RPC 返回
    var mod = mc[moduleKey];
  ...
  function apply(ctx) {
    ...
    // 先获取模块配置，再按开关决定 inject 哪些组件
    // 失败要重试：拿不到配置时所有开关都按「关闭」处理，UI 会一直不出现。
    function loadModuleConfig(attempt) {
      rpc("get-module-config", {}).then(function (mc) {
        if (mc) {
          store.moduleConfig = mc;
          emitChange();
          return;
        }
        retryModuleConfig(attempt);
      }).catch(function () { retryModuleConfig(attempt); });
    }
    function retryModuleConfig(attempt) {
      var n = attempt || 0;
      if (n >= 5) return;
      setTimeout(function () { loadModuleConfig(n + 1); }, Math.min(1000 * (n + 1), 5000));
    }
    loadModuleConfig(0);

    // 提醒卡的滞留时长是 host 侧配置（设置页可改），这里启动时同步一份：
    // client 本地生成的提醒卡（删除/结束/移除失败、测试通知等）不带 autoMs，
    // 按同步到的值消散，避免「设置了 10 秒，本地提示卡还是 4 秒」这种不一致。
    // 失败静默：保留默认 4 秒，不影响主流程。
    rpc("get-config", {}).then(syncCardHoldMs).catch(function () {});

    // ========== 组件包装器：在渲染时检查模块开关 ==========
    // 用 useSyncExternalStore 订阅 store，当配置加载/变更时触发重渲染。
    // store.moduleConfig 为 null（未加载）时默认不渲染，避免组件提前出现。

    function wrapWithModuleCheck(componentFn, moduleKey, childKey) {
      return function Wrapped(props) {
        // 订阅 store 变更，使配置变化时组件重渲染。
        // 注意：子组件必须用 createElement 渲染成独立 fiber。直接调用 componentFn(props)
        // 会把子组件的 hooks 接到本组件的 fiber 上；开关打开时 hook 数量变化，
        // React 抛 #310（Rendered more hooks than during the previous render），
        // DSH 的 slot 错误边界随即把整个 entry 摘掉（abdicate）——
        // 表现就是「模块开关全开，但计时条/任务面板/设置弹窗永远不出现」。
        React.useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
        if (!isModuleOn(moduleKey, childKey)) return null;
        return React.createElement(componentFn, props);
      };
    }

    // ========== 所有 slot 始终注册，工厂函数用包装后的组件 ==========
    // shell.overlay：设置弹窗 + 提醒卡 + 任务面板
    slots.inject("shell.overlay", function () {
      return [
        slots.register({ name: "shell.overlay", id: "task-time-setup", order: 50, label: "任务设置" },
          wrapWithModuleCheck(SetupManager, 'timing', 'planning')),
        slots.register({ name: "shell.overlay", id: "task-time-reminder", order: 60, label: "任务提醒" },
          function ReminderGate(props) {
            React.useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
            // 提醒卡归属「提醒 → 内部通知（在 DSH 里：提醒卡 + 提示音）」这一个开关，
            // 关掉它就应该没有右下角卡片（以前写的是"内部通知 或 定时提醒"都放行，
            // 导致关掉内部通知后卡片照旧弹，和设置页/文档的说法不一致）。
            if (!isModuleOn('alert', 'reminderUI')) return null;
            return React.createElement(ReminderStack, props);
          }),
        slots.register({ name: "shell.overlay", id: "task-time-board", order: 70, label: "任务面板" },
          wrapWithModuleCheck(TaskBoard, 'timing', 'taskBoard')),
      ];
    });

    // Dock 状态条
    slots.inject("conversation.composer.dock", function () {
      return slots.register({ name: "conversation.composer.dock", id: "task-time-status", order: 200, label: "任务计时" },
        function DockGate(props) {
          React.useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
          if (!isModuleOn('timing', 'dockStatus')) return null;
          return React.createElement(DockEntry, props);
        });
    });
    ...
    // 提醒/标题闪烁/浏览器通知 — 始终初始化，运行时检查 isModuleOn
    ...
    function sendBrowserNotif(title, body, sessionId) {
      if (!isModuleOn('alert', 'titleFlash')) return;
      // 浏览器通知也是系统级通知 → 归外部通道：人就在 DSH 里时不发（只走内部通知）
      if (!isModuleOn('alert', 'externalNotify')) return;
      if (inFront) return;
      if (!notifPermissionGranted || !('Notification' in window)) return;
      try {
    ...
    function updateTitleFlash() {
      if (!isModuleOn('alert', 'titleFlash')) {
    ...
    document.addEventListener('visibilitychange', function () {
      if (document.visibilityState === 'visible') onDshWindowActivated();
    });
    window.addEventListener('focus', onDshWindowActivated);
    requestNotifPermission();

    // 在场上报 + 音频解锁（focus/可见性变化时立即上报，轮询里再定期续报）
    document.addEventListener('visibilitychange', function () { reportPresence(true); });
    window.addEventListener('focus', function () { reportPresence(true); });
    window.addEventListener('blur', function () { reportPresence(true); });
    document.addEventListener('pointerdown', unlockAudio);
    document.addEventListener('keydown', unlockAudio);
    reportPresence(true);

    // 客户端 approval/request 监听 — 始终注册，运行时检查 isModuleOn
        ...
        try {
          if (!isModuleOn('alert', 'decisionDetection')) return;
            ...
            if (sid) {
              ...
              notifyInternal({ needDecision: true });
    ...
    var maxId = 0;
    // ---------- 会话日志轮询检测审批请求（仅提醒模块 + decisionDetection 开启） ----------
    var sessionSeqs = {};
    ...
    var poll = setInterval(function () {
      // 提醒获取（界面提醒卡）：仅在提醒模块或定时提醒开启时拉取
      if (isModuleOn('alert', 'reminderUI') || isModuleOn('timing', 'intervalReminder')) {
        rpc("get-reminders", { since: maxId }).then(function (d) {
          if (d && Array.isArray(d.reminders)) {
            var fresh = [];
            d.reminders.forEach(function (r) {
              if (r.id > maxId) { maxId = r.id; fresh.push(r); }
              pushReminder(r);
              if (r.needDecision && r.sessionId) {
                sendBrowserNotif('❓ ' + (r.taskName || '任务') + '：需要你决策', r.text, r.sessionId);
              }
            });
            // 内部通知：在 DSH 里就响一声（不在 DSH 时由 host 发系统通知）
            if (fresh.length) {
              notifyInternal({ needDecision: fresh.some(function (r) { return r.needDecision; }) });
            }
          }
          updateTitleFlash();
        }).catch(function () {});
      }
      // 在场上报：host 用它决定发不发系统通知，返回值同步本地 inFront
      reportPresence();
      // Toast 点击跳转（仅提醒模块 toastJump 子模块开启时）
      if (isModuleOn('alert', 'toastJump')) {
      ...
      // 会话日志轮询检测
      checkSessionForApproval();
      // document.hasFocus() 检测：比 window.focus 事件更可靠地感知用户回到 DSH
      ...
      if (isFocused && !wasFocused) {
        ...
        updateTitleFlash();
        // 用户点了系统通知横幅回到 DSH → 跳到对应会话。
        // 优先消费 toast.ps1 写下的跳转请求；没有请求（例如通知是被忽略的旧横幅、
        // 或 toast 脚本的点击监听没生效）时兜底：直接跳到"正在等你决策"的那个会话，
        // 因为需要你决策本来就是打扰用户回到 DSH 的唯一原因。
        if (isModuleOn('alert', 'toastJump')) {
          rpc("get-pending-jump", {}).then(function (d) {
            if (d && d.sessionId) { jumpToSession(d.sessionId); return; }
            rpc("get-latest-decision-session", {}).then(function (r2) {
              var sid = r2 && r2.sessionId;
              if (!sid) return;
              var cur = null;
              try { var snap = sessionsSvc && sessionsSvc.list && sessionsSvc.list.getSnapshot(); cur = snap && snap.current; } catch (e) {}
              if (sid !== cur) jumpToSession(sid);
            }).catch(function () {});
          }).catch(function () {});
        }
      }
      wasFocused = isFocused;
      // 注意：决策提醒卡现在由 ReminderStack 的会话感知过滤器控制显示/隐藏，
      // 不再每 2 秒全局清除当前会话的决策卡（否会则切走再切回来卡片就没了）。
    }, 2000);
