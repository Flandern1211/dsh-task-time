...
(() => {
  ...
  function apply(ctx) {
    ...
    function sendBrowserNotif(title, body, sessionId) {
      if (!notifPermissionGranted || !('Notification' in window)) return;
      if (document.visibilityState === 'visible' && document.hasFocus()) return; // 用户正在看 DSH 时不发浏览器通知（红色提醒卡已在界面上）
      try {
    ...
    // 用户回到 DSH 窗口时：停止标题闪烁，不清卡不跳转（用户自行点击红卡）
    ...
    // window.focus 作为补充（部分 Electron 环境不触发 visibilitychange）
    ...
    // Track focus state for detecting toast-click activation
