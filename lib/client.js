...
(() => {
  ...
  function pushReminder(r) {
    ...
    var timer = setTimeout(function () { dismissReminder(r.id); }, r.autoMs || 10000);
  ...
  function ReminderStack() {
    ...
    return React.createElement("div", { className: "tt-stack" },
      items.map(function (r) {
        var cls = "tt-reminder" + (r.needDecision ? " tt-need-decision" : "");
  ...
  function TaskBoard() {
    ...
    var activeRows = (data.active || []).map(function (a) {
      ...
      return React.createElement("div", { key: a.sessionId, className: "tt-board-item", onClick: function () { goToSession(a.sessionId); }, title: "点击打开该任务会话" },
        React.createElement("span", { className: "tt-bi-name" }, a.taskName || "未命名任务"),
        tag,
    ...
    var finishedRows = (data.finished || []).slice().reverse().map(function (f) {
      ...
      return React.createElement("div", { key: (f.sessionId || "") + "-" + (f.finishedAt || ""), className: "tt-board-item", onClick: function () { goToSession(f.sessionId); }, title: "点击打开该任务会话" },
        React.createElement("span", { className: "tt-bi-name" }, f.taskName || "未命名任务"),
        tag,
        React.createElement("span", { className: "tt-bi-meta" },
  ...
  function DockEntry(props) {
    ...
    React.useEffect(function () {
      ...
      function refresh() {
        ...
        rpc("get-pending-setup", { sessionId: sessionId }).then(function (d) {
          if (!alive) return;
          if (d && d.pending) {
            setTaskName(d.taskName || "");
            setPlan(String(d.plannedMinutes || "60"));
            setInterval(String(d.reminderIntervalMinutes || "10"));
            setSetup({ sessionId: sessionId });
          } else { setSetup(null); }
        }).catch(function () {});
      }
      refresh();
    ...
    var setupEl = null;
    if (setup) {
      setupEl = React.createElement("div", { className: "tt-setup" },
  ...
  function SetupManager() {
    ...
    React.useEffect(function () {
      ...
      function refresh() {
        if (!alive) return;
        rpc("get-pending-sessions", {}).then(function (d) {
          if (!alive) return;
          var sessions = (d && d.sessions) || [];
          if (sessions.length > 0) {
            var first = sessions[0];
            setTaskName(first.taskName || "");
            setPlan(String(first.plannedMinutes || "60"));
            setInterval(String(first.reminderIntervalMinutes || "10"));
            setSetup({ sessionId: first.sessionId });
          } else { setSetup(null); }
        }).catch(function () {});
  ...
  function SettingsSection() {
    ...
    return React.createElement("div", { className: "tt-settings" },
      ...
      React.createElement("div", { className: "tt-hint" },
        "计时为连续累计：只有模型运行时才计时，暂停（等待你的输入）不计时，恢复后接着累计。创建新会话时可填写任务名并设置计划与提醒间隔（也可直接跳过）；提醒卡片右下角堆叠显示，普通提醒约 10 秒、需决策提醒约 30 秒后自动消散，也可点 × 手动关闭；DSH 需要你决策时弹出红色「需要你决策」提醒；任务结束自动统计累计实际用时与计划用时的差距，并写入本地任务记录文件。左下角「🗂 任务面板」可查看全部任务记录（进行中 / 已完成分组），点击任意任务即可跳转到该任务对应的会话对话。"
      )
    );
  }
